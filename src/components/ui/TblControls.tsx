...
const headCells: HeadCell[] = [
  ...
  { id: 'title', numeric: false, disablePadding: true, label: 'Product Title' },
  { id: 'description', numeric: false, disablePadding: true, label: 'Description' },
  { id: 'packSize', numeric: true, disablePadding: false, label: 'Pack Size' },
  { id: 'basePrice', numeric: true, disablePadding: false, label: 'Price' },
  { id: 'points', numeric: true, disablePadding: false, label: 'Likes' }  
...
export default function TblControls({vendorId}:  props) {
  ...
  let rows = data?.getProducts.products

  const classes = useStyles();
  const [order, setOrder] = React.useState<Order>('asc');
  const [orderBy, setOrderBy] = React.useState<keyof Product>('points');
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      ...
      setSelected(newSelecteds? newSelecteds : [-1]);
      return;
    }
    setSelected([]);
  };
